...
describe("Regression", function () {
  ...
  it("should not add a paragraph after a table if at end of footer / header", function () {
  ...
  it("should work even if having a {#loop} tag inside a Word Field, and the closing tag not in a field", function () {
    /*
     * Fixed since v3.67.3
     * #regression-loop-with-field-and-nofield
     */
    var doc = createDocV4("loop-with-field-and-nofield.docx");
    doc.render({
      loop: [{
        data: "John Doe"
      }, {
...
describe("Assignment", function () {
  ...
  it("should be possible to resolve assignments first", function () {
    var doc = makeDocxV4(paragraph("{@client=(22|getClient)}") + paragraph("{@company=(client|getCompany)}") + paragraph("{company.name}") + "{xx}", {
      modules: [],
      parser: expressionParser.configure({
        ...
        filters: {
          ...
          getCompany: function getCompany(client) {
            return new Promise(function (resolve) {
              setTimeout(function () {
                resolve({
  ...
  it("should work with assignments without angular parser", function () {
    var doc = makeDocxV4(paragraph("{@client=(22|getClient)}") + paragraph("{@company=(client|getCompany)}") + paragraph("{company.name}") + "{xx}");
    return doc.renderAsync({}).then(function () {
...
describe("Prefixes", function () {
    ...
    var doc = makeDocxV4("<w:t>{##tables}{user}{/tables}</w:t>", {
      modules: [{
        optionsTransformer: function optionsTransformer(options, doc) {
          var loopModule = doc.findModule("LoopModule");
  ...
  it("should be possible to change the prefix of the raw xml module to a regexp", function () {
    var doc = makeDocxV4(paragraph("{!!raw}"), {
      modules: [{
        optionsTransformer: function optionsTransformer(options, doc) {
          var rawXmlModule = doc.findModule("RawXmlModule");
          rawXmlModule.prefix = /^!!?(.*)$/;
          return options;
        }
      }]
  ...
  it("should be possible to change the prefix of the raw xml module to a function", function () {
    var content = paragraph("{raw}") + paragraph(" {text}");
    var scope = {
      raw: paragraph("HoHo"),
      text: "Huhu"
    };
    ...
    doc.render(scope);
    expect(doc.zip.file("word/document.xml").asText()).to.be.equal(paragraph("HoHo") + paragraph(" Huhu"));
  });
});
...
describe("Get Tags", function () {
  ...
  it("should correctly get the tags recursively using getObjectIdentifiers", function () {
    var iModule = inspectModule();
    makeDocxV4(paragraph("{#user.age > 12}{user.name}{/}{x+y}{#company.name === 'Acme'}Hi Acme{/}{#list}{priority}{/}"), {
      modules: [iModule]
    });
    function addIdentifiers(tags) {
      tags.forEach(function (tag) {
        ...
        if (tag.cellParsed) {
          tag.cellParsed.forEach(function (cp) {
        ...
        if (tag.type === "placeholder" && typeof tag.value === "string") {
          if (tag.attrParsed) {
            Object.keys(tag.attrParsed).forEach(function (key) {
              addIdentifiers(tag.attrParsed[key]);
            });
            // Skip parsing of value in this case, because the tag is like <v:shape...>
            return;
          }
          var identifiers = expressionParser(tag.value).getObjectIdentifiers();
    ...
    function simplifyTags(tags) {
      ...
      var _loop = function _loop() {
          ...
          if (tag.cellParsed) {
            part = {
              sub: tag.cellParsed.map(function (cp) {
                if (cp.type === "placeholder" && cp.module !== "pro-xml-templating/xls-module-loop") {
                  simplifyTags([cp]);
                }
              }).filter(function (r) {
          ...
          if (tag.type === "placeholder" && typeof tag.value === "string") {
            if (tag.attrParsed) {
              part = {};
              part.sub = Object.keys(tag.attrParsed).map(function (key) {
                return simplifyTags(tag.attrParsed[key]);
              });
              result.push(part);
              // Skip parsing of value in this case, because the tag is like <v:shape...>
              return {
    ...
    var withIdentifiers = addIdentifiers(iModule.getStructuredTags());
    var simplified = simplifyTags(withIdentifiers);
    expect(simplified).to.matchSnapshot();
  });
});
describe("preserveNewlinesInTags option", function () {
  it("should be possible to get splitted tag", function () {
    var iModule = inspectModule();
    ...
    var allTags = iModule.getAllTags();
    expect(Object.keys(allTags)).to.deep.equal(["l1\nl2\n\nl3\n\nl4"]);
  });
  it("should be possible to render template with multiline tag", function () {
    var doc = createDocV4("tag-splitted-multiline.docx", {
      syntax: {
        preserveNewlinesInTags: true
      }
    });
    ...
    shouldBeSame({
      doc: doc,
      expectedName: "expected-john.docx"
    });
  });
  it("should fail with angular parser with multiline template", function () {
    var capture = captureLogs();
    var expectedError = {
    ...
    expectToThrow(function () {
      return createDocV4("tag-splitted-multiline.docx", {
        parser: expressionParser,
        syntax: {
          preserveNewlinesInTags: true
        }
