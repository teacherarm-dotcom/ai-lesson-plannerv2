...
const StandardSearchPopup = ({ isOpen, onClose, providerId, apiKey, onStandardSelected }) => {
      ...
      try {
        ...
        if (text) {
          ...
          if (data?.standards) {
            ...
            if (data.qualificationInfo) {
              setQualificationInfo(data.qualificationInfo);
            }
            addMessage(`น้องเพชรดึงข้อมูลเรียบร้อยแล้วค่ะ! พบ ${data.standards.length} รายการ\n\nตรวจสอบตารางด้านล่างแล้วกดดาวน์โหลดได้เลยค่ะ`, 'bot');
            setStep('done');
  ...
  const generateWordDoc = () => {
    const qi = qualificationInfo || {};
    const fieldName = qi.fieldName || '.......................................................';
    const occupationName = qi.occupationName || '................................................................';
    const level = qi.level || '.......................';
...
<p class="header-info">มาตรฐานอาชีพ  สาขาวิชาชีพ${fieldName}</p>
<p class="header-info">อาชีพ${occupationName}ระดับ${level}</p>
<br/>
...
<tbody>${standardData.map((item) => `<tr><td>${item.uoc_code || ''}</td><td>${item.uoc_desc || ''}</td><td>${item.eoc_code || ''}</td><td>${item.eoc_desc || ''}</td><td>${item.criteria || ''}</td><td>${item.assessment || ''}</td></tr>`).join('')}</tbody></table></body></html>`;
    ...
    // Pass standard data to parent for use in analysis
    if (onStandardSelected) {
      const summaryText = standardData.map((item) =>
        `UoC: ${item.uoc_code} ${item.uoc_desc}\nEoC: ${item.eoc_code} ${item.eoc_desc}\nCriteria: ${item.criteria}\nAssessment: ${item.assessment}`
      ).join('\n\n');
      onStandardSelected(summaryText);
    }

    addMessage('ดาวน์โหลดไฟล์ตารางมาตรฐานเรียบร้อยแล้วค่ะ!\n\nข้อมูลมาตรฐานจะถูกส่งไปใช้ในการวิเคราะห์งานต่อไปค่ะ', 'bot');
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg h-[600px] flex flex-col relative overflow-hidden">
        ...
        {step === 'done' && standardData.length > 0 && (
          <div className="p-2 bg-gray-100 border-t border-gray-200 max-h-40 overflow-y-auto text-xs">
            {qualificationInfo && (
              <div className="bg-blue-50 p-2 rounded mb-2 text-xs text-blue-800">
                <div><strong>สาขาวิชาชีพ:</strong> {qualificationInfo.fieldName || '-'}</div>
                <div><strong>อาชีพ:</strong> {qualificationInfo.occupationName || '-'} <strong>ระดับ:</strong> {qualificationInfo.level || '-'}</div>
              </div>
            )}
        ...
        <div className="p-4 bg-white border-t border-gray-200">
          ...
          {step === 'done' && (
            <button onClick={generateWordDoc} className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2 shadow-lg">
              <FileDown size={20} /> สร้างไฟล์ Word (ตารางมาตรฐาน.doc)
            </button>
          )}
        </div>
      </div>
