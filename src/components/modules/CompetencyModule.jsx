...
import { Zap, CheckCircle, Sparkles, Loader2, Check, ArrowRight, Upload, FileText } from 'lucide-react';
...
const CompetencyModule = ({
  ...
  const generate = async () => {
    ...
    try {
      let parts = [{ text: SYSTEM_PROMPT_COMPETENCY(selectedLevel) }];
      if (hasPreviousData && !compFile) {
      ...
      const data = await callApi(parts, { json: true, statusText: `กำลังวิเคราะห์และเขียนสมรรถนะประจำหน่วย (${selectedLevel})...` });
  ...
  const exportWord = () => {
    ...
    createWordDoc(`สมรรถนะประจำหน่วย_${formData.courseCode}`, `<table><thead><tr><th width="10%">ที่</th><th width="30%">หน่วยการเรียนรู้</th><th>สมรรถนะประจำหน่วย</th></tr></thead><tbody>${rows}</tbody></table>`);
  };
  ...
  const exportPdf = () => {
    ...
    printToPdf(`สมรรถนะประจำหน่วย ${formData.courseCode}`, `<table><thead><tr><th width="10%">ที่</th><th width="30%">หน่วยการเรียนรู้</th><th>สมรรถนะประจำหน่วย</th></tr></thead><tbody>${rows}</tbody></table>`);
  };

  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 min-h-[80vh]">
      <div className="mb-6 border-b border-gray-100 pb-4">
        <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2"><Zap className="text-blue-600" /> สมรรถนะประจำหน่วย (Unit Competencies)</h2>
        <p className="text-gray-500 text-sm mt-1">วิเคราะห์และเขียนสมรรถนะ (ทางปัญญา & ปฏิบัติงาน) ตามระดับหลักสูตร</p>
      </div>
        ...
        <div className="md:col-span-1">
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              {['ปวช.', 'ปวส.'].map((lvl) => (
                <button key={lvl} onClick={() => setSelectedLevel(lvl)} className={`flex-1 py-2 rounded-lg text-sm font-medium border transition ${selectedLevel === lvl ? (lvl === 'ปวช.' ? 'bg-blue-100 border-blue-500 text-blue-700' : 'bg-purple-100 border-purple-500 text-purple-700') : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50'}`}>{lvl}</button>
              ))}
            </div>
          </div>
          ...
          {hasPreviousData ? (
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-6 text-center mb-4">
              <div className="bg-white p-3 rounded-full shadow-sm mb-3 mx-auto w-fit text-purple-600"><CheckCircle size={32} /></div>
              ...
              <button onClick={generate} disabled={loading} className="w-full bg-purple-600 text-white py-3 rounded-xl font-bold hover:bg-purple-700 shadow-lg flex items-center justify-center gap-2">
            ...
            <>
              ...
              <FileUploadZone file={compFile} onUpload={handleCompUpload} label="แนบไฟล์ผลลัพธ์การเรียนรู้" height="h-56" />
              <button onClick={generate} disabled={loading || !compFile} className={`w-full mt-4 py-2.5 rounded-xl font-semibold transition shadow-md flex items-center justify-center gap-2 ${!compFile ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between bg-green-50 p-3 rounded-lg border border-green-200">
                <div className="flex items-center gap-2 text-green-800 text-sm"><Check size={16} /> สร้างสมรรถนะเสร็จสิ้น!</div>
                <ExportButtons onRegenerate={generate} onExportWord={exportWord} onExportPdf={exportPdf} />
              </div>
              <div className="overflow-x-auto border border-gray-200 rounded-xl shadow-sm">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50"><tr><th className="px-4 py-3 text-left text-sm font-bold text-gray-700 w-1/3">หน่วยการเรียนรู้</th><th className="px-4 py-3 text-left text-sm font-bold text-gray-700">สมรรถนะประจำหน่วย</th></tr></thead>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600 align-top leading-relaxed">
                          <ul className="list-none space-y-2">
                            {item.competencies.map((comp, cIdx) => (
                              <li key={cIdx} className="flex gap-2"><span className="font-semibold text-blue-600 min-w-[20px]">{cIdx + 1}.</span><span>{comp.replace(/^\d+\.\s*/, '')}</span></li>
                            ))}
                          </ul>
                        </td>
                      </tr>
              ...
              <div className="mt-8 text-center bg-gray-50 p-6 rounded-xl border border-gray-200">
                ...
                <button onClick={() => onNavigate('objectives')} className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-lg flex items-center gap-2 mx-auto animate-bounce">
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 min-h-[300px] border-2 border-gray-100 rounded-xl border-dashed bg-gray-50">
              <Zap className="w-12 h-12 mb-2 opacity-20" /><p className="text-sm">สมรรถนะประจำหน่วยจะแสดงที่นี่</p>
            </div>
          )}
        </div>
      </div>
